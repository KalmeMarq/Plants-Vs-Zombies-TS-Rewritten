...
class SeedPacket extends Sprite {
  public constructor(almanac: AlamanacPlantsScreens, sprite: Sprite, cost: number) {
    super()
    ...
    const t = this.addChild(new Text(cost.toString(), {
      fill: 0x000000,
      fontSize: 12
    }))
    t.position.set(this.width / 2 - t.width / 2, this.height - t.height - 3)
  }
}
...
export default class AlamanacPlantsScreens extends GUIScreen {
  ...
  public constructor(core: Core) {
    ...
    const s = this.addChild(new SeedPacket(this, Sprite.from('PeaShooter'), 100))
    s.position.set(26, 92)

    const s1 = this.addChild(new SeedPacket(this, Sprite.from('SnowPea'), 175))
    s1.position.set(26 + 52, 92)

    const s3 = this.addChild(new SeedPacket(this, Sprite.from('Sunflower'), 50))
    s3.position.set(26 + 52 + 52 + 52, 92)

    const s2 = this.addChild(new SeedPacket(this, Sprite.from('WallnutBody'), 50))
